    ...
    : ICommandHandler<SendFriendRequestCommand, FriendRequestDto>
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (command.AddresseeUserId == Guid.Empty)
            return Result.Failure<FriendRequestDto>(SocialErrors.UserNotFound);

        if (command.AddresseeUserId == currentUserId)
            return Result.Failure<FriendRequestDto>(SocialErrors.CannotFriendYourself);

        var addressee = await _userRepository.GetByIdAsync(command.AddresseeUserId, cancellationToken);
        if (addressee is null || !addressee.IsActive())
            return Result.Failure<FriendRequestDto>(SocialErrors.UserNotFound);

        if (await _friendshipRepository.AreFriendsAsync(currentUserId, command.AddresseeUserId, cancellationToken))
        ...
        if (reversePending is not null)
            return await AcceptExistingRequestAsync(reversePending, currentUserId, cancellationToken);
        ...
        var request = FriendRequest.Create(Guid.NewGuid(), currentUserId, command.AddresseeUserId, now);
        ...
        await SocialCommandHelpers.PublishFriendEventAsync(
            _socialRealtimePublisher,
            _clock,
            command.AddresseeUserId,
    ...
    private async Task<Result<FriendRequestDto>> AcceptExistingRequestAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        await PublishFriendshipAcceptedAsync(request, currentUserId, dto, cancellationToken);
        return Result.Success(dto);
    }

...
/// <summary>
/// Recommendation generated by the system for a user and task.
/// The entity owns state transitions and core business rules.
/// </summary>
public sealed class TaskRecommendation : EntityBase
{
        ...
        int validHours = 24) : base(id, now)
    {
        ...
        if (score < 0 || score > 100)
            throw new DomainException("Recommendation score must be between 0 and 100.");

        if (validHours <= 0)
            throw new DomainException("Recommendation validity duration must be greater than zero.");
    ...
    public void Accept(DateTimeOffset now)
    {
        ...
        if (Status != StatusTaskRecommendation.Pending)
            throw new DomainException("Only pending recommendations can be accepted.");
    ...
    public void Reject(DateTimeOffset now)
    {
        ...
        if (Status != StatusTaskRecommendation.Pending &&
            Status != StatusTaskRecommendation.Accepted)
        {
            throw new DomainException("The recommendation cannot be rejected in its current state.");
        }
    ...
    public void MarkCompleted(DateTimeOffset now)
    {
        ...
        if (Status != StatusTaskRecommendation.Accepted)
            throw new DomainException("The recommendation must be accepted before it can be completed.");
    ...
    private void EnsureNotExpired(DateTimeOffset now)
    {
        if (ExpiresAt.HasValue && now > ExpiresAt.Value)
        {
            ...
            throw new DomainException("The recommendation has expired.");
        }
